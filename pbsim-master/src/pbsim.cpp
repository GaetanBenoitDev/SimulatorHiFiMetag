...
int main (int argc, char** argv) {
  ...
  while ((opt = getopt_long(argc, argv, "", long_options, &option_index)) != -1) {
    ...
    case 0:
      ...
      case 16:
        sim.coverageFile = new ofstream(optarg);
        (*sim.coverageFile) << "Read;Length;Pos" << endl;
        break;

      case 17:
        sim.headerPrefix = optarg;
        break;
  ...
  for (ref.num=1; ref.num<=ref.num_seq; ref.num++) {
	...
	//resultFile.open("/home/gats/workspace/run/binning_test.csv");
	//resultFile << "GROUP;READ_LENGTH;DIST" << endl;
  ...
  if(sim.coverageFile) sim.coverageFile->close();
...
int simulate_by_sampling() {
  ...
  cout << "SIMULATE BY SAMPLING" << endl;
...
int simulate_by_model() {
  ...
  while (len_total < sim.len_quota) {
    ...
    //cout << sim.headerPrefix << endl;
    sprintf(id, "S%ld_%ld", ref.num, sim.res_num);
    fprintf(fp_fq, "@%s\n%s\n+%s\n%s\n", (sim.headerPrefix + to_string(sim.res_num)).c_str(), mut.new_seq, (sim.headerPrefix + to_string(sim.res_num)).c_str(), mut.new_qc);
    if(sim.coverageFile) (*sim.coverageFile) << id << ";" << strlen(mut.new_seq) << ";" << (mut.seq_left-1) << endl;
...
int mutate() {
  ...
  if (len >= ref.len) {
    ...
    offset = rand() % (ref.len); // + - 1 ?
    //cout << ref.len << " " << rand() % (ref.len) << endl;
    //cout << rand() % 6  << endl;
  }
  ...
  if (sim.res_num % 2 == 0) {
    ...
    for (i=0; i<len; i++) {
      ...
      u_int64_t pos = offset + i;
      if(pos >= ref.len){
        pos -= ref.len;
      }

      nt = toupper(ref.seq[pos]);
      mut.seq[i] = nt;
    }
  } else {
    mut.seq_strand = '-';
  ...
  for (i=0; i<len; i++) {
    ...
    if (sim.accuracy_min < 1 && rand_value < mut.sub_thre[qc_value]) {
      sim.res_sub_num ++;
      index = rand() % 3;
      if (nt == 'A') {
      ...
      mut.maf_ref_seq[maf_offset] = nt;
    } else if (sim.accuracy_min < 1 && rand_value < mut.ins_thre[qc_value]) {
      sim.res_ins_num ++;
      index = rand() % 8;
      if (index >= 4) {
